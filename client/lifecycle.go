...
func registerBackgroundSignals() {
	signal.Notify(
		quit,
		os.Interrupt,
		syscall.SIGTERM,
...
func handleTunnelDeath(t *TUN) {
	defer RecoverAndLog()
	if t == nil || t.GetState() < TunnelConnected {
		return
	}
	meta := t.meta.Load()
	if meta == nil {
	...
	reconnectLoop(t, meta, cr, keepPath, stopCh)
}

func reconnectLoop(t *TUN, meta *TunnelMeta, cr *ConnectionRequest, keepPath bool, stopCh chan struct{}) {
