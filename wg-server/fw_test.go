...
func setupFW(t *testing.T, subnet4, subnet6 string) {
	t.Helper()
	if err := initFirewall(subnet4, subnet6); err != nil {
		t.Fatal(err)
	}
}

func entry(t *testing.T, ip string) *fwPeer {
	t.Helper()
...
func flowLen(p *fwPeer) int {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return len(p.flows)
}

func TestInitFirewall_RejectsLargeSubnet(t *testing.T) {
	if err := initFirewall("10.0.0.0/8", ""); err == nil {
		t.Fatal("subnet larger than /16 must be rejected")
	}
	if err := initFirewall("10.0.0.0/24", ""); err != nil {
...
func TestSetAllowed_ReplaceAndClear(t *testing.T) {
	...
	p.setAllowed(hostRules(t, "10.0.0.11"), false)
	if p.allowedContains(mustAddr(t, "10.0.0.10"), 80) {
...
func TestV6Map_CleanupOnReuse(t *testing.T) {
	setupFW(t, "10.0.0.0/24", "fd00::/64")
	resetPeer("10.0.0.5", "fd00::aaaa")
	resetPeer("10.0.0.5", "fd00::bbbb")
...
func TestParseACLEntry(t *testing.T) {
	for _, tc := range []struct {
		...
		{"10.0.0.5", true, "10.0.0.5", 0, false},
		{"10.0.0.5:22", true, "10.0.0.5", 22, false},
		{"*:443", true, "", 443, true},
		{"fd00::5", true, "fd00::5", 0, false},
		{"[fd00::5]:22", true, "fd00::5", 22, false},
		{"", false, "", 0, false},
		{"10.0.0.5:0", false, "", 0, false},
		{"*:0", false, "", 0, false},
		{"*:bad", false, "", 0, false},
		{"10.0.0.5:99999", false, "", 0, false},
...
func TestSetAllowed_PortRules(t *testing.T) {
	...
	p.setAllowed([]aclEntry{
		...
		{anyHost: true, port: 443},
		{addr: mustAddr(t, "10.0.0.11")},
	}, false)
...
func TestL4Ports(t *testing.T) {
	...
	if sp, dp := l4Ports(1, l4); sp != 0 || dp != 0 {
...
func TestFwPeer_ConcurrentSafe(t *testing.T) {
	setupFW(t, "10.0.0.0/24", "")
	resetPeer("10.0.0.5")
	p := entry(t, "10.0.0.5")
