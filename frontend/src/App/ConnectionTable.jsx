...
const ConnectionTable = () => {
  ...
  const generateStatsTable = () => {
    ...
    state?.ActiveTunnels?.map((cs) => {
      let stocms = Math.floor(String(cs.MS / 1000));
      ...
      row.items.push({
        ...
        align: "left",
        className: "Disk",
        value: String(cs.DISK) + " %",
      });
      row.items.push({
        type: "text",
        align: "left",
        className: "Ping",
        value: dayjs(cs.PingTime).format("HH:mm:ss"),
      });
      row.items.push({
        type: "text",
        align: "left",
  ...
  if (tunnel !== undefined) {
    return (
      <div className="connections">
        <ObjectEditor opts={editorOpts} object={tunnel} />
      </div>
    );
  }
  ...
  if (pserver !== undefined) {
    return (
      <div className="connections">
        <ObjectEditor opts={serverOpts} object={pserver} />
      </div>
    );
  }
